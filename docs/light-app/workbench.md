...
根据不同人员的需求来定义工作台卡片，然后将卡片搭建成不同的工作台配置进行下发，实现“千人千面”。并且支持在移动端对工作台进行管理和定制，支持离线 H5 卡片，应用红点提醒等。
...
::: warning 需要更高级别的定制？
如果需要一个高度定制的工作台，我们推荐使用 H5 的模式进行开发，WorkPlus 支持将 Tab 绑定到访问指定的 H5 页面（或离线资源包）。
:::
